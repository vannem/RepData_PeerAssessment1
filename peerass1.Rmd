...
4. The mean and median of the NA-filled dataset is 10581.01 and 10395. The mean value is different from the previou one, while the median value is still the same. This is because I use the mean for that 5-minute interval to fill in the NA. When calculating mean value, the denominator doesn't change and the numerator increases. The number of values being equal to median increases, while the value doesn't change which is still the mean. 
